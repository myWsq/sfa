...
Unpack SHALL restore mode and mtime for regular files and directories by default, MUST ignore owner restoration for non-root callers unless an explicit preserve-owner policy is enabled, and MUST expose overwrite, owner-restore, and integrity policies through its public API and CLI.
