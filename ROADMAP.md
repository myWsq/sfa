...
`expand-golden-and-regression-assets`

建议范围：

- 扩大 golden fixtures 的协议与语义覆盖面，补齐更大的 regression corpus
- 补齐 CLI 默认值、usage error、stdin / dry-run 组合等行为回归
- 在具备 `lz4` / `zstd` CLI 的环境里补一次 fresh `tar + same codec` 对照，作为真实世界性能旁证而非当前阻塞项
- 维持当前 `node_modules` thread sweep 作为诊断资产；`2026-03-17` 的最新 sweep 已达到 `lz4 4092ms @ t2`、`zstd 3674ms @ t8`，均不劣于此前已知基线
